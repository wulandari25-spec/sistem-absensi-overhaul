                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/30 transition-colors">
                            ...
                            @for($d = 1; $d <= $daysInMonth; $d++)
                                @php
                                    ...
                                    // Jika jadwal kosong, ambil dari shift waktu kehadiran aktual (absensi)
                                    $isActualAttendance = false;
                                    if (!$shift && isset($attendances)) {
                                        $att = $attendances->get($key)?->first();
                                        if ($att) {
                                            $shift = $att->shift;
                                            // Jika shift_id di absensi NULL, cari shift terdekat dari waktu checked_at
                                            if (!$shift && isset($shifts)) {
                                                $currentTime = $att->checked_at->format('H:i:s');
                                                $bestShift = null;
                                                $minDiff = null;
                                                foreach ($shifts as $s) {
                                                    $currentCarbon = \Carbon\Carbon::createFromFormat('H:i:s', $currentTime);
                                                    $shiftStartCarbon = \Carbon\Carbon::createFromFormat('H:i:s', $s->start_time);
                                                    $diff = abs($currentCarbon->diffInMinutes($shiftStartCarbon));
                                                    if ($diff > 720) {
                                                        $diff = 1440 - $diff;
                                                    }
                                ...
                                @endphp
                                <td class="p-1 text-center border-r border-slate-100 dark:border-slate-800/50 {{ (!$shift && !$isWithinContract) ? 'bg-slate-100 dark:bg-slate-950/50 text-slate-400 dark:text-slate-600' : '' }}"
                                    @if(!$isWithinContract && !$shift) title="Di luar masa kontrak kerja pegawai" @endif>
                                    @if($shift)
                                        <span 
                                        ...
                                        >
                                            {{ strtoupper(substr(str_replace('Shift ', '', $shift->name), 0, 1)) }}
                                        </span>
                                    @else
