...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800 dark:text-white">Registrasi Pengguna / Admin Baru</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Daftarkan akun administrator, petugas keamanan, atau petugas K3 baru ke sistem</p>
        </div>
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Misal: Budi Santoso" required
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone') }}" placeholder="Misal: 08123456789"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="budi@overhaul.id" required
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all">
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {{-- Kata Sandi (Password) --}}
                <div class="flex flex-col gap-2">
                    <label for="password" class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Kata Sandi (Password)</label>
