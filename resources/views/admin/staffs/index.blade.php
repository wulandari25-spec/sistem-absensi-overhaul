...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <form method="GET" class="flex flex-wrap items-center gap-3 flex-1 max-w-4xl">
                    ...
                    <tr class="log-row border-b border-slate-50 dark:border-slate-800/50">
                        ...
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-1">
                                <a href="{{ route('admin.staffs.show', $staff) }}" class="p-1.5 rounded-lg text-slate-400 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-950/50 transition-colors" title="Detail">
                                ...
                                @if(!auth()->user()->isK3() && !auth()->user()->isSecurity())
                                    ...
                                    <form action="{{ route('admin.staffs.destroy', $staff) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menonaktifkan pegawai {{ $staff->name }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-1.5 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/50 transition-colors cursor-pointer" title="Hapus / Nonaktifkan">
